...
int SensorAL3006::setEnable(int32_t handle, int en)
{
	...
	LOGD("SensorAL3006::enable en=%d; newState=%d; what=%d; mEnabled=%d\n",\
    ...
    if ((uint32_t(newState) << what) != (mEnabled & (1 << what)))
    {
		...
		LOGD("SensorAL3006::enable what=%d; flags=%d; err=%d\n", what, flags, err);
		...
		LOGD("SensorAL3006::mEnabled=0x%x\n", mEnabled);
...
int SensorAL3006::getEnable(int32_t handle)
{
	...
	LOGD("SensorAL3006::mEnabled=0x%x; enable=%d\n", mEnabled, enable);
...
void SensorAL3006::processEvent(int code, int value)
{
    switch (code) {
        case EVENT_TYPE_PROXIMITY:
            ...
            LOGD("SensorAL3006: mPendingEvents[Proximity].distance = %f",mPendingEvents[Proximity].distance);
            break;        
        case EVENT_TYPE_LIGHT:
            mPendingMask |= 1<<Light;
            mPendingEvents[Light].light = (float)value;
            LOGD("SensorAL3006: mPendingEvents[Light].light = %f",mPendingEvents[Light].light);
            break; 
         default:
            LOGD("SensorAL3006: default value = %d",value);
            break;
    }
}
